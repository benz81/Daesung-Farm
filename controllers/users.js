...
// @request     phone_number, passwd,name,business_license_number,area
// @response    success, token
exports.createUser = async (req, res, next) => {
  let name = req.body.name;
  let phone_number = req.body.phone_number;
  let passwd = req.body.passwd;
  let business_license_number = req.body.business_license_number;
  let area = req.body.area;

  console.log("name" + name);
  console.log("phone_number" + phone_number);
  console.log("passwd" + passwd);
  console.log("business_license_number" + business_license_number);
  console.log("area" + area);

  if (!name || !phone_number || !passwd || !business_license_number || !area) {
    res.status(400).json();
    return;
  }
  ...
  let query =
    "insert into farm_user (name, phone_number, passwd, business_license_number, area) values (?,?,?,?)";
  let data = [name, phone_number, hashedPasswd, business_license_number, area];
  ...
  query = "insert farm_token (user_id, token) values (?,?)";
  data = [user_id, token];
  console.log(
